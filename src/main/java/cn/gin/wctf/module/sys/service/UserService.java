...
import cn.gin.wctf.module.sys.dao.RoleDao;
...
public class UserService {
    ...
    /**
     * <p>查询所有符合条件的用户列表，使用分页。</p>
     * <p>StatusCode: 600 ~ 609</p>
     *
     * @param data - 应用数据流载体
     * @param data$User$user - 包含查询条件的 User 对象，非必需，如果未 null，则没有查询条件限制
     * @param data$Integer$limit - 分页参数，每页显示的数据条数，如果未 null，使用系统默认配置
     * @param data$Integer$page - 分页参数，当前页是第几页，默认为第一页
     */
    public void list(ApplicationDataSupport data) {
        // 1. Prepared the data.
        Integer page = null;
        Integer limit = null;
        User user = null;
        try {
            page = data.getParameter("page");
            limit = data.getParameter("limit");
            user = data.getParameter("user");
        ...
        // 2. Initialize the paging object and user query object.
        Page<User> paging = new Page<User>();
        paging.setIndex(page != null ? page : paging.getIndex());
        paging.setLimit(limit != null ? limit : paging.getLimit());

        // 3. Query the data items count.
        final int count = userDao.count(user);
        paging.setCount(count);
        paging.calculate();

        // 4. Query the data items by pagination.
        user.setPagingCount(count);
        user.setPagingStart((paging.getIndex() - 1) * paging.getLimit());
        user.setPagingLimit(paging.getLimit());
        ...
        // 5. Set the return data format.
        data.clearParams();
        data.setParameter("paging", paging);
        data.setParameter("data", users);
    }

    /**
     * <p>查询所有已删除的用户的用户列表，使用分页。</p>
     ...
     * @param data$Integer$page - 分页参数，当前页是第几页，默认为第一页
     */
    public void listDelete(ApplicationDataSupport data) {
     // 1. Prepared the data.
        Integer page = null;
        Integer limit = null;
        User user = null;
        ...
        // 3. Query the data items count.
        final int count = userDao.countDelete(user);
        paging.setCount(count);
        paging.calculate();
        ...
        user.setPagingStart((paging.getIndex() - 1) * paging.getLimit());
        user.setPagingLimit(paging.getLimit());
        List<User> users = userDao.listDelete(user);
     ...
     *
     * @param data - 应用数据流载体
     * @param data$Integer[]$ids - 需要删除的用户 ID 列表，必须
     */
    public void delete(ApplicationDataSupport data) {
        String[] ids = null;
        try {
            ids = data.getParameter("ids");

            if (ids == null || ids.length == 0) {
                throw new IllegalArgumentException("The id list that need to delete is empty.");
            } else {
                for (String id : ids) {
                    if (!NumberUtils.isNumber(id)) {
                        throw new IllegalArgumentException("The id list contains invalid data.");
                    }
                }
            }
        } catch(Throwable throwable) {
            data.setStatus(612);
            data.setMsg("The ID parameter is invalid");
            return;
        }

        for (String id : ids) {
            userDao.deleteById(NumberUtils.createInteger(id));
        }

        data.setStatus(610);
        data.setMsg("Delete user succeed");
    }
    ...
    public List<Role> getRolesByUserId(Integer userId) {
        ...
        return roleDao.getRolesByUserIdId(userId);
    }
}